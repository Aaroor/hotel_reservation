    ...
    <body data-sa-theme="{{$user_info->getUserInfo(session('user_id'))->theme_id}}">
        <main class="main">
                    	...
                    	<li class="@@colorsactive"><a href="{{route('manager_home')}}"><i class="zmdi zmdi-home"></i> Home</a></li>
                    	 ...
                    	 <li class="navigation__sub navigation__sub--active navigation__sub--toggled">
                    		...
                    		<ul>
                                ...
                                <li class="navigation__active"><a href="{{route('man_available_rooms_index')}}">Availability List</a></li>
            ...
            <section class="content">
                ...
                <div class="card">
                    <div class="card-body">
                        {{--{{Form::open(['action'=>'SuperAdminController@adminAvailableRoomsSearchMap','class'=>'form-horizontal form-label-left input_mask','name'=>'single_booking_form', 'method'=>'POST','role'=>'form','onsubmit'=>'return twoDateSearchValidation()'])}}--}}
                       {{Form::open(['action'=>'ManagerController@manAvailableRoomsSearch','class'=>'form-horizontal form-label-left input_mask','name'=>'edit_res_form', 'method'=>'POST','role'=>'form','onsubmit'=>'return editResFormValidation()'])}}
                        <div class="row">
                            <div class="col-sm-4">
                            ...
                            <table id="data-table" class="table">
                                <thead>
                                    <tr>
                                        ...
                                        <th><i>Booking History</i></th>
                                        <th><i>Floor No</i></th>
                                        <th><i>Room Type</i></th>
                                        <th><i>Status</i></th>
                                        <th><i>Booking Process</i></th>
                                        <th><i>Action</i></th>
                                        {{--<th><i>Action</i></th>--}}
                                    </tr>
                                </thead>
                                <tbody>
                                     @foreach($roomInfos as $roomInfo)
                                        <tr style="font-size: 16px">
                                            ...
                                            <td>
                                             <button class="btn btn-light" data-toggle="modal" data-target="#modal-xl{{$roomInfo->room_id}}"><i style="font-size: 18px" class="zmdi zmdi-info-outline"></i> </button>

                                            </td>
                                            <td>{{$roomInfo->floor_number}}</td>

                                            <td>{{$dec_room_type->getRoomType($roomInfo->room_type)}}</td>
                                            ...
                                            @else
                                                {{--@if($check_out->getCheckOut($is_available->checkAvailable1($from_date,$to_date,$roomInfo->room_id)->invoice_id)->status==1)--}}
                                                {{--<td><span class="badge badge-pill badge-primary">Booked<br>(Paid)</span></td>--}}
                                                {{--@else--}}
                                                <td><span class="badge badge-pill badge-danger">Booked</span></td>
                                                {{--@endif--}}
                                            @endif

                                            @if($is_available->checkProcess($from_date,$to_date,$roomInfo->room_id)==0)
                                                <td><span class="badge badge-pill badge-info"><strong>Waiting</strong><br>&nbsp<strong style="font-size:11px">{{session('from_date')}} To {{session('to_date')}}</strong>&nbsp</span></td>
                                            @elseif($is_available->checkProcess($from_date,$to_date,$roomInfo->room_id)==1)
                                                <td><span class="badge badge-pill badge-info"><strong>In Process</strong><br>&nbsp<strong style="font-size:11px">&nbsp{{$is_available->getBookingDetails1($from_date,$to_date,$roomInfo->room_id)->from_date}} To {{$is_available->getBookingDetails1($from_date,$to_date,$roomInfo->room_id)->to_date}}</strong>&nbsp</span></td>
                                            @else
                                            <td><span class="badge badge-pill badge-info"><strong>Done</strong><br>&nbsp<strong style="font-size:11px">&nbsp{{$is_available->getBookingDetails1($from_date,$to_date,$roomInfo->room_id)->from_date}} To {{$is_available->getBookingDetails1($from_date,$to_date,$roomInfo->room_id)->to_date}}</strong>&nbsp</span></td>
                                                {{--<td><span class="badge badge-pill badge-info">Done</span></td>--}}
                                            @endif



                                            @if($is_available->checkProcess($from_date,$to_date,$roomInfo->room_id)==0)
                                            <td><a href="{{route('man_index_booking_info', ['id'=>$roomInfo->room_id])}}" class="btn btn-light btn--icon-text" ><i class="zmdi zmdi-arrow-forward"></i>For Booking</a></td>
                                            @elseif($is_available->checkProcess($from_date,$to_date,$roomInfo->room_id)==1)
                                            <td><a data-toggle="popover" data-placement="bottom" data-trigger="hover" data-content="Booking Cancel" href="{{route('man_cancel_booking', ['id'=>$remove_id->getBookingIdTwo($from_date,$to_date,$roomInfo->room_id)])}}" class="btn btn-light btn--icon-text" href=""><i class="zmdi zmdi-close"></i></a>
                                            <a data-toggle="popover" data-placement="bottom" data-trigger="hover" data-content="Continue Booking" href="{{route('man_con_booking_info', ['id'=>$roomInfo->room_id,'from'=>$from_date,'to'=>$to_date])}}" class="btn btn-light btn--icon-text" href=""><i class="zmdi zmdi-check"></i></a>
                                            </td>
                                            @else
                                            <td><button class="btn btn-light btn--icon-text" disabled><i class="zmdi zmdi-block"></i>For Booking</button></td>
                                            @endif
