    ...
    <body data-sa-theme="{{$user_info->getUserInfo(session('user_id'))->theme_id}}">
        <main class="main">
                ...
                <div class="scrollbar-inner">
                    ...
                    <ul class="navigation">
                    	...
                    	<li class="navigation__sub navigation__sub--active navigation__sub--toggled">
                    		...
                    		<ul>
                    		   <li class="navigation__active"><a href="{{route('admin_day_summary')}}">Booking Summary</a></li>
            ...
            <section class="content">
                ...
                <div class="card">
                    <div class="card-body">
                    {{Form::open(['action'=>'SuperAdminController@adminSummarySearchP','class'=>'form-horizontal form-label-left input_mask','name'=>'daily_summary_form', 'method'=>'POST','role'=>'form','onsubmit'=>'return dateNullValidation()'])}}
                            ...
                            <div class="col-sm-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input  type="text" name="sum_from_date" class="form-control date-picker" value="{{$sum_date}}">
                    ...
                    {{Form::close()}}
                        ...
                        @inject('a', 'App\Http\Controllers\SuperAdminController')
                        @inject('time_conversion', 'App\Http\Controllers\SuperAdminController')
                        <br><br>
                        <h1>Reservations of this date ({{$sum_date}})</h1>

                        <table class="table mb-3">
                            <thead  class="thead-default">
                            <tr>
                                <th><b>#</b></th>
                                <th><b>Rooms Booking/Meals Order/Airport Booking</b></th>
                                <th><b>Unit Price</b></th>
                                <th><b>Unit</b></th>
                                <th><b>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspAmount</b></th>
                                <hr>
                            ...
                            <tbody>
                            <?php $cont=0;?>
                        @foreach($bookingInvs as $bookingInv)
                            <?php $roomsAmount=0.0;$mealsAmount=0.0;$airBookingAmount=0.0;
                            $roomsBookings=$a->getRoomBookingF($bookingInv->invoice_id);
                            ?>

                            @foreach($roomsBookings as $roomsBooking)
                            <tr>
                                <th scope="row">{{$cont=$cont+1}}</th>
                                <td>Room No : {{$room_info->getRoomDetails($book_info->getBookingInfo($roomsBooking->booking_id)->room_id)->room_number}}<br> Duration : ( {{$book_info->getBookingInfo($roomsBooking->booking_id)->from_date}} To {{$book_info->getBookingInfo($roomsBooking->booking_id)->to_date}})<br>
                                Invoice No : {{$roomsBooking->invoice_id}}<br>
                                <a href="#modal_xl{{$roomsBooking->booking_id}}" data-toggle="modal" data-target="#modal_xl{{$roomsBooking->booking_id}}">More Details ...</a>
                                </td>
                        ...
                        @foreach($orderInvs as $orderInv)

                            <?php $orderPayments=$a->getOrderPaymentF($orderInv->invoice_id); $mealsAmount=0.0?>
                            ...
                            <tr>
                                ...
                                <td>@if($meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_type==4)Drinks Name : @else Meals Name : @endif {{$meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_name}}<br>Order Date : {{$order_info->getOrderDetails($orderPayment->order_id)->order_date}}
                                 @if($order_info->getOrderDetails($orderPayment->order_id)->order_time==null)@else<br> Order Time : {{$time_conversion->timeConversion($order_info->getOrderDetails($orderPayment->order_id)->order_time)}}@endif
                                <br>Invoice No : {{$orderPayment->invoice_id}}<br>
                                <a href="#modal_xl{{$orderPayment->order_pay_id}}" data-toggle="modal" data-target="#modal_xl{{$orderPayment->order_pay_id}}">More Details ...</a>
                                </td>
                            ...
                            @endforeach
                        @endforeach
                        @foreach($airInvs as $airInv)
                            <?php $aBookingPayments=$a->getAirBookingPaymentF($airInv->invoice_id); ?>
                            @foreach($aBookingPayments as $aBookingPayment)
                                <tr>
                                    <th scope="row">{{$cont=$cont+1}}</th>
                                    <td>Airport Booking Date :{{$a_booking->getAirBookingInfo($aBookingPayment->a_booking_id)->a_booking_date}}<br> Booking Time : {{$a_booking->getAirBookingInfo($aBookingPayment->a_booking_id)->a_booking_time}}<br>Invoice No : {{$aBookingPayment->invoice_id}}<br>
                                     <a href="#modal_xl{{$aBookingPayment->a_payment_id}}" data-toggle="modal" data-target="#modal_xl{{$aBookingPayment->a_payment_id}}">More Details ...</a>
                                     </td>
                        ...
                        <br>
                        <h1>Check outs of this date ({{$sum_date}})</h1><br>
                        @if(count($checkOutInvs)!=0)
                        <table class="table mb-3">
                            ...
                            <tbody>
                        <?php $cont=0;?>
                        @foreach($checkOutInvs as $checkOutInv)
                            <?php $roomsAmount=0.0;$mealsAmount=0.0;$airBookingAmount=0.0;
                            $roomsBookings=$a->getRoomBookingF($checkOutInv->invoice_id);
                            ?>
                            ...
                            <tr>
                                ...
                                <td>Room No : {{$room_info->getRoomDetails($book_info->getBookingInfo($roomsBooking->booking_id)->room_id)->room_number}}<br> Duration : ( {{$book_info->getBookingInfo($roomsBooking->booking_id)->from_date}} To {{$book_info->getBookingInfo($roomsBooking->booking_id)->to_date}})<br>
                                Invoice No : {{$roomsBooking->invoice_id}}<br>
                                <a href="#modal_xll{{$roomsBooking->booking_id}}" data-toggle="modal" data-target="#modal_xll{{$roomsBooking->booking_id}}">More Details ...</a>
                                </td>
                                <?php $perAmount=($roomsBooking->total_amount)/$roomsBooking->no_of_days;?>
                                <td>{{number_format((float)$perAmount,2,'.','')}}</td>
                                <td>{{$roomsBooking->no_of_days}}</td>
                        ...
                        @foreach($checkOutInvs as $checkOutInv)
                            ...
                            <?php $orderPayments=$a->getOrderPaymentF($checkOutInv->invoice_id); ?>
                            @foreach($orderPayments as $orderPayment)
                            <tr>
                                <th scope="row">{{$cont=$cont+1}}</th>
                                <td>
@if($meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_type==4)Drinks Name : @else Meals Name : @endif {{$meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_name}}
<br>Order Date : {{$order_info->getOrderDetails($orderPayment->order_id)->order_date}}
@if($order_info->getOrderDetails($orderPayment->order_id)->order_time==null)@else<br> Order Time : {{$time_conversion->timeConversion($order_info->getOrderDetails($orderPayment->order_id)->order_time)}}@endif
<br>Invoice No : {{$orderPayment->invoice_id}}</td>
                                <?php $perMAmount=($orderPayment->amount)/$orderPayment->quantity;?>
                                <td>{{number_format((float)$perMAmount,2,'.','')}}</td>
                                <td>{{$orderPayment->quantity}}</td>
                                <?php $num=30-strlen(number_format((float)$orderPayment->amount,2,'.',''));?>
                        ...
                        @endforeach
                        @foreach($checkOutInvs as $checkOutInv)
                            <?php $aBookingPayments=$a->getAirBookingPaymentF($checkOutInv->invoice_id); ?>
                            @foreach($aBookingPayments as $aBookingPayment)
                                <tr>
                                    <th scope="row">{{$cont=$cont+1}}</th>
                                    <td>Airport Booking Date :{{$a_booking->getAirBookingInfo($aBookingPayment->a_booking_id)->a_booking_date}}<br> Booking Time : {{$a_booking->getAirBookingInfo($aBookingPayment->a_booking_id)->a_booking_time}}<br>Invoice No : {{$aBookingPayment->invoice_id}} </td>
                                    <td>{{number_format((float)$aBookingPayment->amount,2,'.','')}}</td>
                                    <td>1</td>
                                    <?php $num=30-strlen(number_format((float)$aBookingPayment->amount,2,'.',''));?>
                                    <td><?php for($i=0;$i<$num;$i++){echo"&nbsp";}?><b>{{number_format((float)$aBookingPayment->amount,2,'.','')}}</b></td>
                          ...
                          <table class="table table-bordered mb-0" style="font-size: 16px">
                            <thead>
                                <tr style="font-size: 16px">
                                    ...
                                    <th>No check out history available in this date
                                    </th>
                                    <th></th>
                        ...
                        <h1>Payments of this date ({{$sum_date}})</h1><br>

                        <table class="table table-bordered mb-0" style="font-size: 16px">
                        <?php $paidAmount=0.0;?>
                       @if(count($paymentHstys)!=0)
                        {{--@foreach($paymentInvs as $paymentInv)--}}
                            ...
                            @foreach($paymentHstys as $paymentHsty)
                                <thead>
                                    <tr style="font-size: 16px">
                                        ...
                                        <th><b>Payment Date : {{$paymentHsty->add_date}} <br> Payment Type : @if($paymentHsty->payment_type==0) Cash @else Card @endif <br>Invoice No : {{$paymentHsty->invoice_id}}<br>
                                        <a href="#modal_xll{{$paymentHsty->history_id}}" data-toggle="modal" data-target="#modal_xll{{$paymentHsty->history_id}}">More Details ...</a>
                        ...
                        @else
                            <thead>
                                <tr style="font-size: 16px">
                                    ...
                                    <th>No payment history in this date
                                    </th>
                                    <th></th>
                         ...
                         <table class="table table-bordered mb-0" style="font-size: 16px">
                            <thead>
                                <tr style="font-size: 16px">
                                    ...
                                    <th><b>Total Paid Amount (Date : {{$sum_date}})</b></th>
                 ...
                 @foreach($bookingInvs as $bookingInv)
                    ...
                    ?>
                    @foreach($roomsBookings as $roomsBooking)
                    <div class="modal fade" id="modal_xl{{$roomsBooking->booking_id}}" tabindex="-1">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <table class="table mb-3">
                                        ...
                                        <tbody>
                                        <?php $cont=0;$roomsAmount=0.0;$mealsAmount=0.0;$airBookingAmount=0.0; ?>
                                        @foreach($booking_pay->getBookingPay($roomsBooking->invoice_id) as $roomsBooking)
                                        <tr>
                                            <th scope="row">{{$cont=$cont+1}}</th>
                                        ...
                                        @foreach($order_pay->getOrderPay($roomsBooking->invoice_id) as $orderPayment)
                                        <tr>
                                            <th scope="row">{{$cont=$cont+1}}</th>
                                            <td>@if($meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_type==4)Drinks Name : @else Meals Name : @endif {{$meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_name}}<br>Order Date : {{$order_info->getOrderDetails($orderPayment->order_id)->order_date}}
                                        ...
                                        @endforeach
                                        @foreach($air_pay->getAirBookingPay($roomsBooking->invoice_id) as $aBookingPayment)
                                            <tr>
                                                <th scope="row">{{$cont=$cont+1}}</th>
                                    ...
                                    <table class="table table-bordered mb-0">
                                        ...
                                        @if(count($hsty_pay->getPayHistory($roomsBooking->invoice_id))!=0)

                                        @foreach($hsty_pay->getPayHistory($roomsBooking->invoice_id) as $paymentHsty)
                                            <thead>
                                                <tr style="font-size: 16px">
                                                    <th>#</th>
                                                    <?php $paidAmount=$paidAmount+$paymentHsty->paid_amount;?>
                 ...
                 @foreach($orderInvs as $orderInv)
                     ...
                     <?php $orderPayments=$a->getOrderPaymentF($orderInv->invoice_id); ?>
                     @foreach($orderPayments as $orderPayment)
                        <div class="modal fade" id="modal_xl{{$orderPayment->order_pay_id}}" tabindex="-1">
                            <div class="modal-dialog modal-xl">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title pull-left">Bill Preview</h5>
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <table class="table mb-3">
                                            ...
                                            <tbody>
                                            <?php $cont=0;$roomsAmount=0.0;$mealsAmount=0.0;$airBookingAmount=0.0; ?>
                                            @foreach($booking_pay->getBookingPay($orderPayment->invoice_id) as $roomsBooking)
                                            <tr>
                                                <th scope="row">{{$cont=$cont+1}}</th>
                                                <td>Room No : {{$room_info->getRoomDetails($book_info->getBookingInfo($roomsBooking->booking_id)->room_id)->room_number}}<br> Duration : ( {{$book_info->getBookingInfo($roomsBooking->booking_id)->from_date}} To {{$book_info->getBookingInfo($roomsBooking->booking_id)->to_date}})</td>
                                                <?php $perAmount=($roomsBooking->total_amount)/$roomsBooking->no_of_days;?>
                                            ...
                                            @foreach($order_pay->getOrderPay($orderPayment->invoice_id) as $orderPayment)
                                            <tr>
                                                <th scope="row">{{$cont=$cont+1}}</th>
                                                <td>@if($meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_type==4)Drinks Name : @else Meals Name : @endif {{$meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_name}}<br>Order Date : {{$order_info->getOrderDetails($orderPayment->order_id)->order_date}}
                                                @if($order_info->getOrderDetails($orderPayment->order_id)->order_time==null)@else<br> Order Time : {{$time_conversion->timeConversion($order_info->getOrderDetails($orderPayment->order_id)->order_time)}}@endif</td>
                                            ...
                                            @endforeach
                                            @foreach($air_pay->getAirBookingPay($orderPayment->invoice_id) as $aBookingPayment)
                                                <tr>
                                                    <th scope="row">{{$cont=$cont+1}}</th>
                                                    <td>Booking Date :{{$a_booking->getAirBookingInfo($aBookingPayment->a_booking_id)->a_booking_date}}<br> Booking Time : {{$a_booking->getAirBookingInfo($aBookingPayment->a_booking_id)->a_booking_time}} </td>
                                                    <td>{{number_format((float)$aBookingPayment->amount,2,'.','')}}</td>
                                        ...
                                        <table class="table table-bordered mb-0">
                                            ...
                                            @if(count($hsty_pay->getPayHistory($orderPayment->invoice_id))!=0)

                                            @foreach($hsty_pay->getPayHistory($orderPayment->invoice_id) as $paymentHsty)
                                                <thead>
                                                    <tr style="font-size: 16px">
                                                        <th>#</th>
                                                        <?php $paidAmount=$paidAmount+$paymentHsty->paid_amount;?>
                 ...
                 @foreach($airInvs as $airInv)
                     <?php $aBookingPayments=$a->getAirBookingPaymentF($airInv->invoice_id); ?>
                     @foreach($aBookingPayments as $aBookingPayment)
                        <div class="modal fade" id="modal_xl{{$aBookingPayment->a_payment_id}}" tabindex="-1">
                            <div class="modal-dialog modal-xl">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title pull-left">Bill Preview</h5>
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <table class="table mb-3">
                                            ...
                                            <tbody>
                                            <?php $cont=0;$roomsAmount=0.0;$mealsAmount=0.0;$airBookingAmount=0.0; ?>
                                            @foreach($booking_pay->getBookingPay($aBookingPayment->invoice_id) as $roomsBooking)
                                            <tr>
                                                <th scope="row">{{$cont=$cont+1}}</th>
                                                <td>Room No : {{$room_info->getRoomDetails($book_info->getBookingInfo($roomsBooking->booking_id)->room_id)->room_number}}<br> Duration : ( {{$book_info->getBookingInfo($roomsBooking->booking_id)->from_date}} To {{$book_info->getBookingInfo($roomsBooking->booking_id)->to_date}})</td>
                                                <?php $perAmount=($roomsBooking->total_amount)/$roomsBooking->no_of_days;?>
                                            ...
                                            @endforeach
                                            <?php $discount=$checkOut->getCheckOutInfo($invId)->discount_amount;$cont=0.0;$mealsAmount=0.0?>

                                            @foreach($order_pay->getOrderPay($aBookingPayment->invoice_id) as $orderPayment)
                                            <tr>
                                                <th scope="row">{{$cont=$cont+1}}</th>
                                                <td>@if($meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_type==4)Drinks Name : @else Meals Name : @endif {{$meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_name}}<br>Order Date : {{$order_info->getOrderDetails($orderPayment->order_id)->order_date}}
                                                @if($order_info->getOrderDetails($orderPayment->order_id)->order_time==null)@else<br> Order Time : {{$time_conversion->timeConversion($order_info->getOrderDetails($orderPayment->order_id)->order_time)}}@endif</td>
                                            ...
                                            @endforeach
                                            @foreach($air_pay->getAirBookingPay($aBookingPayment->invoice_id) as $aBookingPayment)
                                                <tr>
                                                    <th scope="row">{{$cont=$cont+1}}</th>
                                                    <td>Booking Date :{{$a_booking->getAirBookingInfo($aBookingPayment->a_booking_id)->a_booking_date}}<br> Booking Time : {{$a_booking->getAirBookingInfo($aBookingPayment->a_booking_id)->a_booking_time}} </td>
                                                    <td>{{number_format((float)$aBookingPayment->amount,2,'.','')}}</td>
                                        ...
                                        <table class="table table-bordered mb-0">
                                            ...
                                            @if(count($hsty_pay->getPayHistory($aBookingPayment->invoice_id))!=0)

                                            @foreach($hsty_pay->getPayHistory($aBookingPayment->invoice_id) as $paymentHsty)
                                                <thead>
                                                    <tr style="font-size: 16px">
                                                        <th>#</th>
                                                        <?php $paidAmount=$paidAmount+$paymentHsty->paid_amount;?>
                 ...
                 @foreach($checkOutInvs as $checkOutInv)
                     ...
                     ?>
                     @foreach($roomsBookings as $roomsBooking)
                        <div class="modal fade" id="modal_xll{{$roomsBooking->booking_id}}" tabindex="-1">
                            <div class="modal-dialog modal-xl">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title pull-left">Bill Preview</h5>
                 ...
                 @foreach($paymentHstys as $paymentHstyA)
                    <div class="modal fade" id="modal_xll{{$paymentHstyA->history_id}}" tabindex="-1">
                        <div class="modal-dialog modal-xl">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title pull-left">Bill Preview ({{$paymentHstyA->invoice_id}})</h5>
                                </div>
                                <div class="modal-body">
                                     @inject('countDay', 'App\Http\Controllers\SuperAdminController')
                                      @inject('book_info', 'App\Http\Controllers\SuperAdminController')
                                    ...
                                    <table class="table mb-3">
                                        ...
                                        <tbody>
                                        <?php $cont=0;$roomsAmount=0.0;$mealsAmount=0.0;$airBookingAmount=0.0; ?>
                                        @foreach($booking_pay->getBookingPay($paymentHstyA->invoice_id) as $roomsBooking)
                                        <tr>
                                            <th scope="row">{{$cont=$cont+1}}</th>
                                            <td>Room No : {{$room_info->getRoomDetails($book_info->getBookingInfo($roomsBooking->booking_id)->room_id)->room_number}}<br> Duration : ( {{$book_info->getBookingInfo($roomsBooking->booking_id)->from_date}} To {{$book_info->getBookingInfo($roomsBooking->booking_id)->to_date}})</td>
                                            <?php $perAmount=($roomsBooking->total_amount)/$roomsBooking->no_of_days;?>
                                        ...
                                        @foreach($order_pay->getOrderPay($paymentHstyA->invoice_id) as $orderPayment)
                                        <tr>
                                            <th scope="row">{{$cont=$cont+1}}</th>
                                            <td>@if($meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_type==4)Drinks Name : @else Meals Name : @endif {{$meals_info->getMealsInfo($order_info->getOrderDetails($orderPayment->order_id)->meals_id)->meals_name}}<br>Order Date : {{$order_info->getOrderDetails($orderPayment->order_id)->order_date}}
                                            @if($order_info->getOrderDetails($orderPayment->order_id)->order_time==null)@else<br> Order Time : {{$time_conversion->timeConversion($order_info->getOrderDetails($orderPayment->order_id)->order_time)}}@endif</td>
                                        ...
                                        @endforeach
                                        @foreach($air_pay->getAirBookingPay($paymentHstyA->invoice_id) as $aBookingPayment)
                                            <tr>
                                                <th scope="row">{{$cont=$cont+1}}</th>
                                                <td>Booking Date :{{$a_booking->getAirBookingInfo($aBookingPayment->a_booking_id)->a_booking_date}}<br> Booking Time : {{$a_booking->getAirBookingInfo($aBookingPayment->a_booking_id)->a_booking_time}} </td>
                                                <td>{{number_format((float)$aBookingPayment->amount,2,'.','')}}</td>
                                    ...
                                    <table class="table table-bordered mb-0">
                                        ...
                                        @if(count($hsty_pay->getPayHistory($paymentHstyA->invoice_id))!=0)

                                        @foreach($hsty_pay->getPayHistory($paymentHstyA->invoice_id) as $paymentHsty)
                                            <thead>
                                                <tr style="font-size: 16px">
                                                    <th>#</th>
                                                    <?php $paidAmount=$paidAmount+$paymentHsty->paid_amount;?>
