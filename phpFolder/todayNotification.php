...
try {
  ...
  // $queryCheckOut="SELECT * FROM booking_info WHERE ? between from_date and to_date and to_date=? AND check_out_status=0 ORDER BY update_date //DESC"; 
    $queryCheckOut="SELECT * FROM booking_info WHERE DATE(to_date)=? AND check_out_status=0 ORDER BY update_date DESC";
    $queryAir="SELECT * FROM air_port_booking_info WHERE DATE(a_booking_date)=? ORDER BY update_date DESC";
    $queryMeals="SELECT * FROM order_info WHERE DATE(order_date)=? ORDER BY update_date DESC";
    $query = "SELECT * FROM booking_info WHERE ? between from_date and to_date AND check_out_status=0 ORDER BY update_date DESC";
...
$stmt->bindParam(1,$date_time);
    ...
    if($num>0){
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
            ...
            $val= $val."<a href='https://system.nellymarine.com/receptionist/res_booking_detail/{$booking_id}' class='listview__item'>
...
$stmt1->bindParam(1,$date_time);
    ...
    if($num1>0){
        ...
        while ($row1 = $stmt1->fetch(PDO::FETCH_ASSOC)){
            ...
            $val= $val."<a href='https://system.nellymarine.com/receptionist/res_order_detail/{$order_id}' class='listview__item'>
...
$stmt2->bindParam(1,$date_time);
    ...
    if($num2>0){
        while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)){
            ...
            $val= $val."<a href='https://system.nellymarine.com/receptionist/res_air_detail/{$a_booking_id}' class='listview__item'>
    ...
    $stmt3 = $conn->prepare($queryCheckOut);
    $stmt3->bindParam(1,$date_time);
   // $stmt3->bindParam(2,$date_time);
    $stmt3->execute();
    $num3 = $stmt3->rowCount();
    $val=$val."<br><h6 style='text-align:center'><b>Check Outs</b>&nbsp&nbsp<span class='badge badge-secondary'><b><i>{$num3}</i></b></span></h6>";
    if($num3>0){
        while ($row3 = $stmt3->fetch(PDO::FETCH_ASSOC)){
            extract($row3);
            $roomInf1=getData($conn);
            $roomInf1->bindParam(1,$room_id);
            $roomInf1->execute();
            // store retrieved row to a variable
            $rowC = $roomInf1->fetch(PDO::FETCH_ASSOC);

            $val= $val."<a href='https://system.nellymarine.com/receptionist/res_pay_check/{$invoice_id}' class='listview__item'>
                                            <img src='https://system.nellymarine.com/demo/img/profile-pics/check_out.jpg' class='listview__img' alt=''>

                                            <div class='listview__content'>
                                                <div class='listview__heading'>Room No :{$rowC['room_number']}</div>
                                                <p>Invoice No : {$invoice_id}</p>
                                                <p>Floor No : {$rowC['floor_number']} </p>
                                                <p>{$from_date} To {$to_date}</p>
                                            </div>
                                        </a>";
            ...
            $val=$val."<a href='' class='listview__item'>
                                            ...
                                            <div class='listview__content'>
                                                <div class='listview__heading'>No Check Out Data Available</div>
                                            </div>
                                        </a>";
        }
